...
Fmapの直下には選択中のvision別検査項目について、表示中の各lotの生の測定値分布をKDEとしてlotごとに表示する。各KDEは対応するFmapのlotと同じ横幅で並べ、FQmap・Fmapと横スクロール位置を同期する。同じ検査項目では全lotでx軸・y軸の尺度を共通にする。両側規格では規格中心を横軸中央、規格下限を左端から1/6、規格上限を5/6に固定し、3本の縦線を表示する。片側上限規格では架空の規格中心を設けず、`meta_best`を左端、規格上限を5/6に固定する。異物・欠陥の測定値は0以上とし、負の横軸を表示しない。固定軸の範囲外にある測定の最大割合はKDE左欄へ表示する。
