...
現在の概要画面では、全検査項目を俯瞰する領域として、各`lot_number × FrameNo × colname`について次の3段をFQmapと呼ぶ。
...
FQmapの下には、選択中の1検査項目だけを扱う詳細領域としてFmapとKDEを1つのカードにまとめる。Fmapの左欄に`Fmap`の名称と選択中のvision別検査項目を1回だけ明示し、その検査項目について`PositionX = 1～24`、`PositionY = 1～12`の製品位置マップを次の3段で表示する。
...
Fmapの直下には選択中のvision別検査項目について、表示中の各lotの生の測定値分布をKDEとしてlotごとに表示する。各KDEは対応するFmapのlotと同じ横幅で並べ、FQmap・Fmapと横スクロール位置を同期する。列が同じlotを表すことはFQmap最上段のlot番号で示し、KDE上にはlot番号を重複表示しない。同じ検査項目では全lotでx軸・y軸の尺度を共通にする。両側規格では規格中心を横軸中央、規格下限を左端から1/6、規格上限を5/6に固定し、3本の縦線を表示する。片側上限規格では架空の規格中心を設けず、`meta_best`を左端、規格上限を5/6に固定する。異物・欠陥の測定値は0以上とし、負の横軸を表示しない。固定軸の範囲外にある測定の最大割合はKDE左欄へ表示する。

画面はライトテーマとし、3段のFQmap間には6pxの淡い区切り帯を設けて各指標を識別しやすくする。全項目の俯瞰であるFQmapと、選択項目の詳細であるFmap・KDEには異なる配色とグルーピングを用いる。FQmap、Fmap、KDEの高さ比は`config.toml`の設定に従い、FQmapの3段を可能な限り同時に表示する。Escキーでウィンドウを閉じる。
